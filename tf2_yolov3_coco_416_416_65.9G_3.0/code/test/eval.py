...
def yolo_predict_keras(model, image, anchors, num_classes, model_input_shape, nms_iou_threshold, conf_threshold, elim_grid_sense, v5_decode):
    image_data = preprocess_image(image, model_input_shape)
    #origin image shape, in (height, width) format
    image_shape = image.size[::-1]
    ...
    if len(anchors) == 5:
        ...
        if v5_decode:
            ...
            # pred_boxes, pred_classes, pred_scores = yolo3_postprocess_np(prediction, image_shape, anchors, num_classes, model_input_shape, max_boxes=100, confidence=conf_threshold, elim_grid_sense=elim_grid_sense)
            pred_boxes, pred_classes, pred_scores = yolo3_postprocess_np(prediction, image_shape, anchors, num_classes, model_input_shape, max_boxes=100, iou_threshold=nms_iou_threshold, confidence=conf_threshold, elim_grid_sense=elim_grid_sense)

    return pred_boxes, pred_classes, pred_scores


def get_prediction_class_records(model, model_format, annotation_records, anchors, class_names, model_input_shape, nms_iou_threshold, conf_threshold, elim_grid_sense, v5_decode, save_result):
    '''
    ...
    for (image_name, gt_records) in annotation_records.items():
        ...
        # normal keras h5 model
            ...
            pred_boxes, pred_classes, pred_scores = yolo_predict_keras(model, image, anchors, len(class_names), model_input_shape, nms_iou_threshold, conf_threshold, elim_grid_sense, v5_decode)
        else:
...
def eval_AP(model, model_format, annotation_lines, anchors, class_names, model_input_shape, eval_type, iou_threshold, nms_iou_threshold, conf_threshold, elim_grid_sense, v5_decode, save_result, class_filter=None):
    '''
    ...
    pred_classes_records = get_prediction_class_records(model, model_format, annotation_records, anchors, class_names, model_input_shape, nms_iou_threshold, conf_threshold, elim_grid_sense, v5_decode, save_result)
    AP = 0.0
...
def main():
    ...
    parser.add_argument(
        ...
        help='evaluation type (VOC/COCO), default=%(default)s', default='')
    ...
    parser.add_argument(
        '--nms_iou_threshold', type=float,
        help='IOU threshold for NMS, default=%(default)s', default=0.5)
    ...
    parser.add_argument('--quant',
                        default=False,
                        action="store_true",
                        help="if is quantized model")
    parser.add_argument('--eval_only',
                        default=False,
                        action="store_true",
                        help="if eval quantized model only")
    ...
    if args.quant:
        ...
        calib_dataset = _get_calib_dataset()
        batch_size = 20
        steps = 4952 // 20
        if args.dump:
            with vitis_quantize.quantize_scope():
                quant_model = tf.keras.models.load_model("./quantized/quantized.h5")
                vitis_quantize.VitisQuantizer.dump_model(
                      quant_model,
                      dataset=next(calib_dataset),
            ...
            quant_model.summary()
            model_format = "H5"
            eval_AP(quant_model, model_format, annotation_lines, anchors, class_names, model_input_shape, args.eval_type, args.iou_threshold, args.nms_iou_threshold, args.conf_threshold, args.elim_grid_sense, args.v5_decode, args.save_result, class_filter=class_filter)
            end = time.time()
            print("Evaluation time cost: {:.6f}s".format(end - start))
        else:
            print("DEBUG: Loaded float model from:", args.model_path) # ADD THIS LINE
            model, model_format = load_eval_model(args.model_path)
            # model.predict(calib_dataset, batch_size=batch_size, steps=steps)
            print("DEBUG: Starting VitisQuantizer.quantize_model()...") # ADD THIS LINE    

            quant_model = vitis_quantize.VitisQuantizer(model,
                    'pof2s').quantize_model(calib_dataset=calib_dataset,
                            calib_batch_size=batch_size,
                            calib_steps=steps)
            print("DEBUG: VitisQuantizer.quantize_model() finished. Type of quant_model:", type(quant_model)) # ADD THIS LINE                                     
            output_save_path = os.path.join('./quantized', 'quantized.h5')
        print(f"DEBUG: Attempting to save quantized model to: {output_save_path}") # ADD THIS LINE
        try: # ADD THIS TRY-EXCEPT BLOCK
            quant_model.save(output_save_path)
            print(f"DEBUG: Successfully saved quantized model to: {output_save_path}") # ADD THIS LINE
        except Exception as e:
            print(f"ERROR: Failed to save quantized model: {e}") # ADD THIS LINE
            # Optional: re-raise the exception if you want it to crash on save failure
            # raise
        ...
        start = time.time()
        model.summary()
        eval_AP(model, model_format, annotation_lines, anchors, class_names, model_input_shape, args.eval_type, args.iou_threshold, args.nms_iou_threshold, args.conf_threshold, args.elim_grid_sense, args.v5_decode, args.save_result, class_filter=class_filter)
        end = time.time()
        print("Evaluation time cost: {:.6f}s".format(end - start))
