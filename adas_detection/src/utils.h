...
void detect(vector<vector<float>>& boxes, vector<float> result, int channel,
  ...
  if (model_type == 0) {      
      ...
      box.push_back(exp(swap[h * width + w][c][2]) * biases[2 * c + 6 * num] / float(sWidth)); // 6 for 3 anchors * 2 values
      box.push_back(exp(swap[h * width + w][c][3]) * biases[2 * c + 6 * num + 1] / float(sHeight)); // 6 for 3 anchors * 2 values
  }
        box.push_back(-1);
